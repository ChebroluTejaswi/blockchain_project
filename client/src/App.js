...
class App extends Component {
  state = {
    ...
    contract: null,
    availableRooms: 0,
    rooms: ["Yes", "Yes", "Yes", "Yes", "Yes", "Yes", "Yes", "Yes"],
  };
    ...
    return (
      <body display="grid">
        <h1 align="center" classname="header">
          Hotel reservations Blockchain
        </h1>
        <h4 align="center">Book Rooms Privately</h4>
        <p align="center">Made by - Chebrolu Tejaswi 19MID0116</p>
            ...
            {this.state.rooms.map((roomAvailability, index) => {
              return (
                <tr key={index} onClick={() => this.bookRoom(index)}>
                  <td className="toggler">{index + 1}</td>

                  <td>{roomAvailability}</td>
                </tr>
              );
            })}
          </tbody>
